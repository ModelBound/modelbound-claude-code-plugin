...
async function main() {
  ...
  if (sub === "create") {
    ...
    const name = nameIdx >= 0 ? process.argv[nameIdx + 1] : void 0;
    const prompt = promptIdx >= 0 ? process.argv[promptIdx + 1] : void 0;
    if (!name || !prompt) {
  ...
  if (sub === "run") {
    ...
    const evalCaseId = caseIdx >= 0 ? process.argv[caseIdx + 1] : void 0;
    const actual = outIdx >= 0 ? process.argv[outIdx + 1] : void 0;
  ...
  if (sub === "results") {
    ...
    const args = { limit: 50 };
    if (caseIdx >= 0)
      args.eval_case_id = process.argv[caseIdx + 1];
