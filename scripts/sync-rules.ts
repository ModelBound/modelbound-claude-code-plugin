// Pull workspace-scoped skills from cloud into local paths (.claude/, .modelbound/, etc.).
...
const PULL_PREFIXES = [".claude/", ".modelbound/", ".cursor/rules/", ".kiro/skills/"];

function shouldPull(sourcePath: string): boolean {
...
async function main() {
  ...
  const data = await callMcpTool<{ skills?: SkillRow[]; items?: SkillRow[] }>(
    cfg,
    apiKey,
    "list_skills",
    {},
  ...
  for (const row of targets) {
    ...
    if (!id) continue;
    const fetched = await callMcpTool<SkillBody>(
      cfg,
      apiKey,
      "get_skill",
      { skill_id: id },
