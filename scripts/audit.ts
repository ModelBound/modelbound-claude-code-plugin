// Scan .claude/ + .mcp.json for: leaked secrets, prompt-injection patterns,
// untrusted MCP URLs. Reports locally and posts to ModelBound audit-log.
import { promises as fs } from "node:fs";
import { join, relative } from "node:path";
import { callMcpTool, loadConfig } from "./config.js";
...
interface Finding { path: string; kind: string; detail: string; }
...
async function walk(dir: string, out: string[] = []): Promise<string[]> {
  ...
  for (const e of entries) {
    ...
    else if (/\.(md|mdc|json|txt|ya?ml)$/i.test(e.name)) out.push(p);
  }
  return out;
}
...
async function main() {
  ...
  for (const f of files) {
    ...
    for (const { name, re } of SECRET_PATTERNS) {
      if (re.test(content)) findings.push({ path: rel, kind: "secret", detail: name });
    }
    for (const { name, re } of INJECTION_PATTERNS) {
      if (re.test(content)) findings.push({ path: rel, kind: "prompt-injection", detail: name });
    }
    if (rel.endsWith(".mcp.json")) {
      try {
        ...
        for (const [name, srv] of Object.entries((cfg.mcpServers ?? {}) as Record<string, any>)) {
          ...
          if (url.startsWith("http://")) findings.push({ path: rel, kind: "mcp", detail: `${name} uses plaintext HTTP` });
          if (/localhost|127\.0\.0\.1|10\.|192\.168\./.test(url)) findings.push({ path: rel, kind: "mcp", detail: `${name} points to a private network address` });
        }
      } catch { /* ignore parse */ }
    }
  }
